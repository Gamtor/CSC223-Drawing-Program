...
import java.lang.*;
public class ultimateBlurringVersion17 extends JFrame implements ActionListener,MouseListener{
    int gridx = 50;
    int gridy = 40;
    ...
    // int[] saveCode = new int[(gridx+2)*(gridy+2)];
    ...
    public void actionPerformed(ActionEvent e){ // runs when an actionlistener is called.
        ...
        switch(cmd){ // When this item is selected, do this.
            ...
            case "Load":
                ...
                //double[] loadCode = new double[(gridx+2)*(gridy+2)];
                ...
                // loadCode[i] = Math.round(Integer.parseInt(stringToInt[i]));
                // }
                int counter=0;
                for (int y=0; y<gridy+2; y++){
                    for(int x = 0; x<gridx+2; x++){ // this sets all grids to empty
                        System.out.println(Math.round(Float.parseFloat(stringToInt[counter])));
                        // try{
                            // loadCode[x][y] = Math.round(Integer.parseInt(stringToInt[counter]));
    ...
    public ultimateBlurringVersion17(){
