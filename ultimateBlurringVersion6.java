...
public class ultimateBlurringVersion6 extends JFrame implements ActionListener,MouseListener{
    int gridx = 40;
    int gridy = 32;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if(e.getX()/30<=gridx && e.getY()/30<=gridy){
            mousex=Math.round(e.getX()/20); // current mouse X pos
            mousey=Math.round(e.getY()/20); // current mouse Y pos
        }
    ...
    public void paint (Graphics g) {
        ...
        int colorGridX=0;
        int colorGridY=0;
        for(int red=0; red<25; red++){
            for(int green=0; green<25; green++){
                for(int blue=0; blue<25; blue++){
                    Color currentColor = new Color(red*20,green*20,blue*20);
                    g2.setColor(currentColor);
                    if(colorGridY<25){
                        g2.fillRect(colorGridX*width+xPos,(colorGridY*length+yPos)+colorOffset,width,length);
                    }
                    repeats += 1;
                    colorGridX++;
                    if(colorGridX>=25){
    ...
    public ultimateBlurringVersion6(){
